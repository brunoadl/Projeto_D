...
%%
load('vetor.mat');
sinal = -vetor;%INVERTIDO PQ O SINAL PPG É NORMALMENTE ANALIZADO INVERTIDO
sinal2 = -vetor; %inverteu o sinal
L = length(sinal); %L � o tamnnho do sinal
vetor_x = [1:1:length(sinal)]'; % criou uma vetor coluna com o npumero de cada linha
vetor_x_derivada = [1:1:(length(sinal)-1)]'; % criou um vetor coluna com o nu=�mero de cada linha menos 1
...
P1 = Yabs(1 : L/2+1);
P1(2 : end-1) = 2*P1(2 : end-1); %fft da vari�vel "sinal"

f = Fs*(0 : (L/2))/L;

%% FILTRAGEM
% wp_A = 0.00008*pi;  %frequência de passagem 0,01 Hz (0,01/128)*pi
% ws_A = 0.04*pi;  %frequência de rejeição 5 Hz (5/128)*pi
% Ap_A = 0.1;  %atenuação de passagem
% As_A = 50; %atenuação de rejeição
...
% 
% wp_B = 0.08*pi;  %frequência de passagem 10 Hz (10/128)*pi
% ws_B = 0.15*pi;  %frequência de rejeição 20 Hz (20/128)*pi
% Ap_B = 0.1;  %atenuação de passagem
% As_B = 50; %atenuação de rejeição
...
%passou a vari�vel "sinal2" pelo filtro passa alta e depois pelo filtro passa baixa
...
%criou um vetor com a derivada do sinal - diferen�a entre o pr�ximo ponto e
%o atual
...
for i = 1 : length(derivada)-1
   if derivada(i+1)>0 && derivada(i)<0
      ...
      if distancia_zero_cross > 80 %verifica se a distancia do zero-cross anterior é maior do 70 amostras
...
%tever 1 eler soma 1 no vetor_ciclos e mantem repetidno este n�ermo at� o
...
x_pico_atual = 0;
y_pico_atual = 0;
vetor_tempo_picos = zeros(vetor_ciclos(end), 1); %cria um vetor para armezenar os tempos de cada um dos picos encontrados, vetor com o mesmo n�mero de linhas que o n�mero de picos 

for i = 1 : vetor_ciclos(end)%vai repetir o mesmo n�ero de vezes que apareceu um pico
    vetor_id_ciclo = vetor_ciclos == i;
    y_ciclo_atual = Filtrado_PB_2(vetor_id_ciclo);
    x_ciclo_atual = vetor_x(vetor_id_ciclo);
    [x_pico_atual, y_pico_atual] = detectar_pico(x_ciclo_atual, y_ciclo_atual); %testa de o pr[�ximo ponto � um queda ap�s o pico
...
% 
% subplot(4,1,2);
% %plot(vetor_x, Filtrado_PB_2);%soma e multiplicação para deixar alinhados no plot
...
function h = filtro_passa_alta(wp, ws, Ap, As)
    ...
    Deltaw = ws - wp;
    omegac = (ws+wp)/2;
    numDeCoef = ceil(6.6*pi/Deltaw)+1; %ceil arredonda e somou 1 para ficar com número ímpar de coeficientes
    M = numDeCoef-1;
